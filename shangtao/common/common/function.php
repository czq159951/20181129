...
function WSTConf($key,$v = ''){
	if(is_null($v)){
		if(array_key_exists('SHANGTAOCONF',$GLOBALS) && array_key_exists($key,$GLOBALS['SHANGTAOCONF'])){
		    unset($GLOBALS['SHANGTAOCONF'][$key]);
		}
	}else if($v === ''){
		if(array_key_exists('SHANGTAOCONF',$GLOBALS)){
			$conf = $GLOBALS['SHANGTAOCONF'];
			$ks = explode(".",$key);
		...
		return $GLOBALS['SHANGTAOCONF'][$key] = $v;
	}
	return null;
}
...
function WSTWxMessage($params){
	...
	if($tpl && file_exists('shangtao'.DS.'wechat'.DS.'behavior'.DS.'InitWechatMessges.php')){
		Hook::exec(['shangtao\\wechat\\behavior\\InitWechatMessges','run'],$params);
	}
}
/**
 * 批量发送微信消息
...
function WSTWxBatchMessage($params){
    $tpl = WSTMsgTemplates($params['CODE']);
	if($tpl && file_exists('shangtao'.DS.'wechat'.DS.'behavior'.DS.'InitWechatMessges.php')){
		\think\facade\Hook::exec(['shangtao\\wechat\\behavior\\InitWechatMessges','batchRun'],$params);
	}
}
/**
 * 截取字符串
...
function WSTShopEncrypt($shopId){
	return md5(base64_encode("shangtao".date("Y-m-d").$shopId));
}
/**
 * 根据子分类循环获取其父级分类
 */
