...
Page({
  ...
  */
  ...
  getTotal:function(){
    ...
    var sessionId = app.globalData.confInfo.sessionId;
    var tokenId = app.globalData.tokenId;
    http.Post("weapp/carts/getQuickCartMoney", { tokenId: tokenId, isUseScore: isUseScore, useScore: data.userOrderScore,sessionId: sessionId}, function (res) {
      if (res.status == 1){
        that.setData({
          price: res.data,
        })
  ...
  submit:function(){
    ...
    if (shopRemarks[data.carts.shopId]) {
      datas['remark_' + data.carts.shopId] = shopRemarks[data.carts.shopId];
    } else {
      datas['remark_' + data.carts.shopId] = '';
    }
